...
const CreateGrn = () => {
  ...
  const [isLoadingProducts, setIsLoadingProducts] = useState(false); // Add loading state
  ...
  const validateForm = () => {
    ...
    if (!formData.preparedBy.trim()) errors.preparedBy = "Prepared By is required";
    ...
    formData.items.forEach((item, index) => {
      ...
      if (!item.receivedQuantity || parseFloat(item.receivedQuantity) <= 0)
        errors[`items[${index}].receivedQuantity`] = "Valid quantity is required";
      ...
      if (item.damageStatus === "Damage") {
        if (!item.goodQuantity || parseFloat(item.goodQuantity) < 0)
          errors[`items[${index}].goodQuantity`] = "Valid good quantity is required";
        if (!item.damageQuantity || parseFloat(item.damageQuantity) <= 0)
          errors[`items[${index}].damageQuantity`] = "Valid damage quantity is required";
    ...
    if (formData.gst_percentage === "" || isNaN(parseFloat(formData.gst_percentage))) {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'freight' || name === 'gst_percentage') {
      processedValue = Math.max(0, isNaN(parseFloat(value)) ? '' : parseFloat(value));
    }
    ...
    try {
      const payload = {
        ...
        items: formData.items.map((item) => ({
          ...
          good_quantity: item.damageStatus === "Damage"
            ? parseFloat(item.goodQuantity) || 0
            : parseFloat(item.receivedQuantity) || 0,
          damage_quantity: item.damageStatus === "Damage"
            ? parseFloat(item.damageQuantity) || 0
            : 0,
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/grn`, {
        method: "POST",
        headers: {
      ...
      if (result.success && result.data) {
        setGrnNumber(result.data.grnNo);
        setSubmissionSuccess(true);
        setTimeout(() => navigate("/adminpurchase/all-grn"), 3000);
      }
    } catch (error) {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-white">
      <div className="flex flex-col items-center mb-4">
        ...
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-black">GOODS RECEIPT NOTE</h2>
          <p className="text-md text-blue-600">
            <span className="font-semibold">GRN No:</span> {formData.grnNo || ""}
          </p>
        </div>
      </div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className={`w-full p-2 border ${
                    validationErrors.grnDate ? "border-red-500" : "border-gray-300"
                  } rounded focus:ring-2 focus:ring-blue-500`}
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className={`w-full p-2 border ${
                    validationErrors.preparedBy ? "border-red-500" : "border-gray-300"
                  } rounded focus:ring-2 focus:ring-blue-500`}
                />
 ...
 <td className="px-2 py-2 border">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (e.target.value.length >= 2) {
                ...
                if (foundProduct) {
                  handleItemChange(index, "goodsDescription", foundProduct.description);
                }
              }
            }}
            placeholder="00"
            className={`w-full p-1 border ${
              validationErrors[`items[${index}].sku`] ? "border-red-500" : "border-gray-300"
            } rounded text-sm`}
          />
        </div>
          ...
          {validationErrors[`items[${index}].goodsDescription`]}
                    ...
                    <td className="px-2 py-2 border">
                      <select
                        ...
                        className={`w-full p-1 border ${
                          validationErrors[`items[${index}].uom`] ? "border-red-500" : "border-gray-300"
                        } rounded text-sm`}
                      >
                    ...
                    <td className="px-2 py-2 border">
                      {item.damageStatus === "Damage" ? (
                        ...
                        <input
                          ...
                          className={`w-full p-1 border ${
                            validationErrors[`items[${index}].receivedQuantity`] ? "border-red-500" : "border-gray-300"
                          } rounded text-sm`}
                        />
                      )}
                    ...
                    <td className="px-2 py-2 border">
                      <input
                        ...
                        className={`w-full p-1 border ${
                          validationErrors[`items[${index}].perUnitCost`] ? "border-red-500" : "border-gray-300"
                        } rounded text-sm`}
                      />
            ...
            <div className="border border-gray-300 rounded p-4">
              <div className="">
                ...
                <div className="flex justify-between">
                  ...
                  <span>₹{(calculateTotal() - calculateDamageAmount() + parseFloat(formData.freight || 0)).toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span>GST ({formData.gst_percentage}%):</span>
                  <span>
                    ₹{(
                      (calculateTotal() - calculateDamageAmount() + parseFloat(formData.freight || 0)) * 
          ...
          <div className="border border-gray-300 rounded p-4">
            <div className="space-y-2">
              ...
              <div className="flex justify-between border-t pt-1">
                ...
                <span>₹{(calculateTotal() - calculateDamageAmount()).toFixed(2)}</span>
              </div>
  ...
  <div className="flex flex-col items-end">
    ...
    {validationErrors.gst_percentage && (
      <p className="text-red-500 text-xs mt-1 text-right">
        {validationErrors.gst_percentage}
      </p>
    )}
