...
const AllProducts = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/partner/dashboard`,
          {
            method: "GET",
            headers: {
  ...
  const handleUpdate = (companyId) => {
    navigate(`/salesuser/update-product/${companyId}`);
  };
  ...
  if (loading) {
    return <div className="salesuser-container loading">Loading...</div>;
  }

  if (error) {
    return (
      <div className="salesuser-container error">
  ...
  if (data.length === 0) {
    return (
      <div className="salesuser-container no-data">No company data available</div>
    );
  }

  return (
    <div className="salesuser-container">
      <div className="salesuser-header">
        <h1 className="salesuser-title">Created Order Lists</h1>
        <div className="search-container">
          ...
          <input
            ...
            className="search-input_09"
          />
        </div>
      </div>

      <div className="table-wrapper">
        <table className="salesuser-table">
          <thead>
            <tr>
      ...
      {totalPages > 1 && (
        <div className="salesuser-pagination">
          <button
            onClick={prevPage}
